...
設計の詳細と、なぜそうなっているかは [`docs/DESIGN.md`](docs/DESIGN.md) を参照。
...
送信元制限があり J-LIS との契約なしには到達できない ([`docs/DESIGN.md`](docs/DESIGN.md) §13.7)。

自分の証明書の有効性確認 (J-LIS の CVS) も**実装していない**。一度は実装して実カードで
測定したが、本番系でもテスト系でも返るのは JPKI 独自コード `401` — 公式クライアントの
コード表にも名前が無い値で、意味が分からない。利用者に返せる答えが無い機能なので削除した。
プロトコルの解析結果は [`docs/DESIGN.md`](docs/DESIGN.md) §13 に残してある。
自分の証明書の有効性確認は JPKI 利用者ソフトを使うのが正規。
